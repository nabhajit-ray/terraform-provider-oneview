...
func resourceApplianceTimeandLocal() *schema.Resource {
	return &schema.Resource{
		Create: resourceApplianceTimeandLocalCreate,
		Read:   resourceApplianceTimeandLocalRead,
...
func resourceApplianceTimeandLocalCreate(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)

	applianceTimeandLocal := ov.ApplianceTimeandLocal{
		Locale:         d.Get("locale").(string),
		DateTime:       d.Get("date_time").(string),
		TimeZone:       d.Get("time_zone").(string),
	}
	rawntpServers := d.Get("ntp_servers").(*schema.Set).List()
	ntpServers := make([]string, len(rawntpServers))
	for i, raw := range rawntpServers {
		ntpServers[i] = raw.(string)
	}
	applianceTimeandLocal.NtpServers = ntpServers

	ApplianceTimeError := config.ovClient.CreateApplianceTimeandLocal(applianceTimeandLocal)
	if ApplianceTimeError != nil {
		return ApplianceTimeError
	}
	return resourceApplianceTimeandLocalRead(d, meta)
}

func resourceApplianceTimeandLocalRead(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
